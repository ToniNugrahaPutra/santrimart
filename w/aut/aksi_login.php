...
$query = "SELECT * FROM tabel_member WHERE nm_user = ? AND kd_toko = ?";
$stmt = mysqli_prepare($koneksi, $query);
mysqli_stmt_bind_param($stmt, "ss", $nama, $_SESSION['kd_toko']);
	...
	echo '<script>
				setTimeout(function() {
					Swal.fire({
						icon: "error",
						tittle: "Gagal login",
						text: "Akun Tidak ditemukan di toko anda",
					}).then(function() {
						window.location = "login.php";
					});
				}, 1);
			</script>';
} else if ($passmd != $data['password']) {
	echo '<script>
				setTimeout(function() {
					Swal.fire({
						icon: "error",
